...
abi-decoder request (./src/commands/abi-decoder/request.ts)
`,
  ];
  ...
  // e.g. ./bin/dev abi-decoder request --hex 0x...
  async run(): Promise<void> {
    const { flags } = await this.parse(Request);

    this.log('run abi-decoder request! (./src/commands/abi-decoder/request.ts)');
    // [Debug]
    // this.log(flags.data);
